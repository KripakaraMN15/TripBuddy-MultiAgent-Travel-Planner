...
def _run_async_compatible(coro):
    """Run a coroutine safely whether the app is in an active event loop or not."""
    try:
...
def flight_agent(state: TravelState):
    ...
    try:
        airports = _run_async_compatible(aviation_mcp_call("list_airports"))
        airlines = _run_async_compatible(aviation_mcp_call("list_airlines"))
...
def hotel_agent(state: TravelState):
    ...
    try:
        hotel_results = _run_async_compatible(
...
# Weather Agent - original behavior kept, MCP call added
...
def weather_agent(state: TravelState):
    ...
    try:
        weather_data = _run_async_compatible(
            weather_mcp_search(city)
        )

        forecast_data = _run_async_compatible(
...
"""
    ...
    cleaned_itinerary = _clean_generated_text(str(response.content))
...
"""
    ...
    cleaned_final_response = _clean_generated_text(str(response.content))
...
# PostgreSQL Checkpointer - falls back to in-memory when no live DB is configured.

def _build_checkpointer():
    database_url = get_database_url()
    if not database_url:
        print("WARNING: DATABASE_URL not configured. Using in-memory checkpoint store.")
        return MemorySaver()
    ...
    try:
        ...
        print(f"WARNING: PostgreSQL connection failed ({exc}). Falling back to in-memory checkpoint store.")
        return MemorySaver()
